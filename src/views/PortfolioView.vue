<template>
  <main class="work-page">
    ...
    <section class="portfolio-carousel" tabindex="0" aria-label="Portfolio projects carousel" @keydown.left.prevent="previousProject" @keydown.right.prevent="nextProject">
      ...
      <div class="carousel-controls"><button class="carousel-arrow" type="button" aria-label="Previous project" @click="previousProject"><svg class="arrow-icon" viewBox="0 0 16 16" aria-hidden="true"><path d="M13 8H3M7 4l-4 4 4 4" /></svg></button><span>{{ String(activeIndex + 1).padStart(2, '0') }} / {{ String(items.length).padStart(2, '0') }}</span><button class="carousel-arrow" type="button" aria-label="Next project" @click="nextProject"><svg class="arrow-icon" viewBox="0 0 16 16" aria-hidden="true"><path d="M3 8h10M9 4l4 4-4 4" /></svg></button></div>

      <article class="project-info glass-panel"><div class="project-info-top"><p class="project-type">{{ activeProject.type }}</p><span class="project-year">{{ activeProject.year }}</span></div><div class="project-info-title"><h2>{{ activeProject.name }}</h2><a v-if="activeProject.demo !== 'null'" :href="activeProject.demo" target="_blank" rel="noreferrer">View project <svg class="arrow-icon" viewBox="0 0 16 16" aria-hidden="true"><path d="M3 13 13 3M6 3h7v7" /></svg></a></div><p>{{ activeProject.status }}</p><div class="project-dots" aria-label="Choose a project"><button v-for="(item, index) in items" :key="item.id" type="button" :class="{ active: index === activeIndex }" :aria-label="'Show ' + item.name" :aria-current="index === activeIndex ? 'true' : undefined" @click="selectProject(index)"></button></div></article>
    </section>
  </main>
</template>
